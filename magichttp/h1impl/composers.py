...
from typing import Optional, Mapping, Union, Iterable, Tuple, MutableMapping
...
def compose_request_initial(
    method: constants.HttpRequestMethod, *, uri: str,
    ...
    if "connection" not in refined_headers.keys() and \
            version == constants.HttpVersion.V1_0:
...
def compose_response_initial(
    ...
    if "connection" not in refined_headers.keys():
        ...
        elif req_initial and req_initial.headers.get_first(
                "connection", "").lower() == "close":
            refined_headers["connection"] = "Close"

    if "transfer-encoding" not in refined_headers.keys() and \
        "content-length" not in refined_headers.keys() and (
        req_initial is None or (req_initial.method not in (
            constants.HttpRequestMethod.HEAD,
            constants.HttpRequestMethod.CONNECT) and
            status_code not in (
