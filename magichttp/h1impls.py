...
class H1ClientStreamManager(
    ...
    def __init__(self, __impl: "H1ClientImpl", buf: bytearray) -> None:
        ...
        self._composer = h1composers.H1RequestComposer(
            self._impl._using_https)
        self._parser = h1parsers.H1ResponseParser(
            self._buf, using_https=self._impl._using_https)

        self._request_initial_bytes: Optional[bytes] = None

        self._writer: Optional[writers.HttpRequestWriter] = None
        self._writer_ready = asyncio.Event()
        self._write_finished = False
    ...
    def _data_arrived(self) -> None:
        ...
        if self._reader is None:
            ...
            self._reader = readers.HttpResponseReader(
    ...
    def write_request(
        ...
        self._writer = writers.HttpRequestWriter(
    ...
    def write_data(self, data: bytes, finished: bool=False) -> None:
        ...
        if self._request_initial_bytes:
            data = self._request_initial_bytes + data
            self._request_initial_bytes = None
    ...
    async def flush_buf(self) -> None:
        ...
        if self._request_initial_bytes:
            data = self._request_initial_bytes
            self._request_initial_bytes = None
...
class H1ClientImpl(BaseH1Impl, protocols.HttpClientProtocolDelegate):
    def __init__(
        ...
        self._stream_mgr = H1ClientStreamManager(self, self._buf)
